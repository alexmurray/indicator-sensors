...
void
is_log_set_level(IsLogLevel level)
{
...
void
is_log(const gchar *source, IsLogLevel level, const gchar *format, ...)
{
  va_list args;
...
is_log_level_to_string(IsLogLevel level)
{
  ...
  g_return_val_if_fail(
      (int)level >= IS_LOG_LEVEL_ERROR && level < NUM_IS_LOG_LEVELS, NULL);
...
void
is_logv(const gchar *source, IsLogLevel level, const gchar *format,
        va_list args)
{
  if (level <= is_log_level)
  {
    ...
    fmt = g_strdup_printf("[%s] %s: %s", source, is_log_level_to_string(level),
                          format);
