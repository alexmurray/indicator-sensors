...
typedef struct _IsPreferencesDialogPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE(IsPreferencesDialog, is_preferences_dialog,
                           GTK_TYPE_DIALOG);
...
is_preferences_dialog_class_init(IsPreferencesDialogClass *klass)
{
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("application", "application property",
                          "application property blurp.", IS_TYPE_APPLICATION,
                          ...
                          IsPreferencesDialog *self)
{
  ...
  priv = is_preferences_dialog_get_instance_private(self);

  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(priv->celsius_radio_button)))
  {
...
static void
sensor_properties_clicked_cb(GtkButton *button, gpointer data)
{
...
is_preferences_dialog_init(IsPreferencesDialog *self)
{
  ...
  gtk_window_set_title(GTK_WINDOW(self),
                       _("Hardware Sensors Indicator Preferences"));
  gtk_window_set_default_size(GTK_WINDOW(self), 500, 600);

  priv->sensor_properties_button =
      gtk_button_new_with_mnemonic(_("_Properties"));
  ...
  markup = g_strdup_printf("<span weight='bold'>%s</span>", _("General"));
  ...
  priv->autostart_check_button =
      gtk_check_button_new_with_label(_("Start automatically on login"));
  gtk_widget_set_sensitive(priv->autostart_check_button, FALSE);
  gtk_grid_attach(GTK_GRID(priv->grid), priv->autostart_check_button, 0, 1, 3,
                  1);
  ...
  gtk_box_pack_start(GTK_BOX(box), priv->display_icon_check_button, TRUE, TRUE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(box), priv->display_label_check_button, TRUE, TRUE,
                     0);

  priv->display_value_check_button =
      gtk_check_button_new_with_label(_("Value"));
  gtk_widget_set_sensitive(priv->display_value_check_button, FALSE);
  gtk_box_pack_start(GTK_BOX(box), priv->display_value_check_button, TRUE, TRUE,
                     0);

  gtk_grid_attach(GTK_GRID(priv->grid), box, 1, 2, 2, 1);
  ...
  gtk_widget_set_margin_start(label, 1);
  gtk_widget_set_margin_end(label, 1);
  gtk_grid_attach(GTK_GRID(priv->grid), label, 0, 3, 1, 1);

  priv->celsius_radio_button =
      gtk_radio_button_new_with_label(NULL, _("Celsius (\302\260C)"));
  gtk_widget_set_sensitive(priv->celsius_radio_button, FALSE);
  gtk_widget_set_hexpand(priv->celsius_radio_button, TRUE);
  gtk_grid_attach(GTK_GRID(priv->grid), priv->celsius_radio_button, 1, 3, 1, 1);
  g_signal_connect(priv->celsius_radio_button, "toggled",
                   G_CALLBACK(temperature_scale_toggled), self);
  priv->fahrenheit_radio_button = gtk_radio_button_new_with_label_from_widget(
  ...
  gtk_grid_attach(GTK_GRID(priv->grid), priv->fahrenheit_radio_button, 2, 3, 1,
                  1);
...
static void
is_preferences_dialog_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec)
{
  IsPreferencesDialog *self = IS_PREFERENCES_DIALOG(object);
  IsPreferencesDialogPrivate *priv =
      is_preferences_dialog_get_instance_private(self);
...
update_flags_sensitivities(IsPreferencesDialog *self)
{
  ...
  IsIndicatorDisplayFlags flags =
      g_settings_get_int(priv->indicator_settings, "display-flags");
...
static void
display_icon_toggled(GtkToggleButton *toggle_button, IsPreferencesDialog *self)
{
  IsPreferencesDialogPrivate *priv =
      is_preferences_dialog_get_instance_private(self);
  ...
  g_settings_set_int(priv->indicator_settings, "display-flags", flags);
  update_flags_sensitivities(self);
}

static void
display_label_toggled(GtkToggleButton *toggle_button, IsPreferencesDialog *self)
{
  IsPreferencesDialogPrivate *priv =
      is_preferences_dialog_get_instance_private(self);
...
static void
display_value_toggled(GtkToggleButton *toggle_button, IsPreferencesDialog *self)
{
  IsPreferencesDialogPrivate *priv =
      is_preferences_dialog_get_instance_private(self);
...
static void
settings_display_flags_changed(GSettings *settings, gchar *key,
                               IsPreferencesDialog *self)
{
  IsPreferencesDialogPrivate *priv =
      is_preferences_dialog_get_instance_private(self);
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(priv->display_icon_check_button),
      flags & IS_INDICATOR_DISPLAY_ICON);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(priv->display_label_check_button),
      flags & IS_INDICATOR_DISPLAY_LABEL);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(priv->display_value_check_button),
      flags & IS_INDICATOR_DISPLAY_VALUE);
}

static void
autostart_toggled(GtkToggleButton *toggle_button, IsApplication *application)
{
...
static void
application_notify_autostart(IsApplication *application, GParamSpec *pspec,
                          ...
                          IsPreferencesDialog *self)
{
  ...
  sensor = is_manager_get_selected_sensor(
      is_application_get_manager(priv->application));
  if (sensor)
  {
    sensitive = TRUE;
    g_object_unref(sensor);
  }
  gtk_widget_set_sensitive(priv->sensor_properties_button, sensitive);
}

static void
manager_row_activated(GtkTreeView *tree_view, GtkTreePath *path,
                      GtkTreeViewColumn *column, IsPreferencesDialog *self)
{
...
static void
is_preferences_dialog_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec)
{
  ...
  case PROP_APPLICATION:
    ...
    flags = g_settings_get_int(priv->indicator_settings, "display-flags");
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(priv->display_icon_check_button),
        flags & IS_INDICATOR_DISPLAY_ICON);
    gtk_toggle_button_set_active(
    ...
    g_signal_connect(gtk_tree_view_get_selection(GTK_TREE_VIEW(manager)),
                     "changed", G_CALLBACK(manager_selection_changed), self);
    ...
    case IS_TEMPERATURE_SENSOR_SCALE_CELSIUS:
      gtk_toggle_button_set_active(
          GTK_TOGGLE_BUTTON(priv->celsius_radio_button), TRUE);
      break;

    case IS_TEMPERATURE_SENSOR_SCALE_FAHRENHEIT:
      gtk_toggle_button_set_active(
          GTK_TOGGLE_BUTTON(priv->fahrenheit_radio_button), TRUE);
      break;
    ...
    gtk_container_add(GTK_CONTAINER(scrolled_window), GTK_WIDGET(manager));
    gtk_widget_set_hexpand(scrolled_window, TRUE);
    gtk_widget_set_vexpand(scrolled_window, TRUE);
    gtk_grid_attach(GTK_GRID(priv->grid), scrolled_window, 0, 4, 3, 1);
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec);
    break;
...
GtkWidget *
is_preferences_dialog_new(IsApplication *application)
{
  return GTK_WIDGET(g_object_new(IS_TYPE_PREFERENCES_DIALOG, "application",
                                 application, NULL));
}
